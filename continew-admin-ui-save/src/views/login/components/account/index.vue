<template>
  ...
  >
    ...
    <a-form-item hide-label>
      <div class="login-mode-group">
        <div
          :class="['login-mode-btn', { 'login-mode-btn-checked': loginMode === 'user' }]"
        ...
        <div
          :class="['login-mode-btn', { 'login-mode-btn-checked': loginMode === 'admin' }]"
...
import type { ViewMode } from '@/stores/modules/user'
...
// 登录模式：user-用户登录，admin-管理员登录
const loginMode = ref<ViewMode>('admin')
// 是否启用验证码
...
const handleLogin = async () => {
  try {
    ...
    // 设置视图模式
    userStore.setViewMode(loginMode.value)
    ...
    if (redirect) {
      ...
      // 根据登录模式跳转到不同首页
      const targetPath = loginMode.value === 'user' ? '/user-home' : '/'
      await router.push({
        path: targetPath,
        query: {
          ...othersQuery,

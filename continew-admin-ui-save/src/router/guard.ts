  ...
  router.beforeEach(async (to, from, next) => {
    ...
    if (getToken()) {
      if (to.path === '/login') {
        ...
        // 根据视图模式跳转到对应首页
        const homePath = userStore.viewMode === 'user' ? '/user-home' : '/'
        next(homePath)
      } else {
        if (!hasRouteFlag) {
          try {
            ...
            // 用户视图下，如果访问管理员专属页面，重定向到用户首页
            if (userStore.viewMode === 'user') {
              const adminOnlyPaths = ['/', '/dashboard', '/dashboard/workplace', '/dashboard/analysis']

<template>
  <a-row justify="end" align="center">
    <a-space size="medium">
      ...
      <!-- 视图切换 -->
      <a-tooltip :content="userStore.isAdminView ? '切换到用户视图' : '切换到管理员视图'" position="bottom">
        <a-button size="mini" class="gi_hover_btn" @click="toggleViewMode">
          <template #icon>
            <icon-user v-if="userStore.isAdminView" :size="18" />
            <icon-settings v-else :size="18" />
          </template>
        </a-button>
      </a-tooltip>
...
const toggleViewMode = () => {
  const newMode = userStore.isAdminView ? 'user' : 'admin'
  Modal.confirm({
    title: '切换视图',
    content: `确认切换到${newMode === 'user' ? '用户' : '管理员'}视图？`,
    onOk: async () => {
      userStore.setViewMode(newMode)
      // 刷新页面以重新加载路由
      window.location.href = newMode === 'user' ? '/user-home' : '/'
    },
  })
}
